...
DDS::DomainId_t
OpenDDS::DCPS::TransportInterface::get_domain_id() const
{
  // Subclass should override if interested in the
  // disassociate_* operations.
  DBG_ENTRY_LVL("TransportInterface","get_domain_id",6);
  return Service_Participant::ANY_DOMAIN;
}

OpenDDS::DCPS::RepoId
OpenDDS::DCPS::TransportInterface::get_participant_id() const
{
  // Subclass should override if interested in the
  // disassociate_* operations.
  DBG_ENTRY_LVL("TransportInterface","get_participant_id",6);
  return GUID_UNKNOWN;
}
...
void
OpenDDS::DCPS::TransportInterface::disassociate_participant(
  RepoId remote_id) // participant
{
  DBG_ENTRY_LVL("TransportInterface","disassociate_participant",6);

  DDS::DomainId_t domain_id(get_domain_id());
  RepoId local_id(get_participant_id());

  if (DCPS_debug_level >= 4) {
    RepoIdConverter local_converter(local_id);
    RepoIdConverter remote_converter(remote_id);
    ACE_DEBUG((LM_DEBUG,
               ACE_TEXT("(%P|%t) TransportInterface::disassociate_participant:")
               ACE_TEXT(" disassociating local participant %C from remote")
               ACE_TEXT(" participant %C.\n"),
  ...
  try {
    ...
    repo->disassociate_participant(domain_id,
                                   local_id,
                                   remote_id);

  } catch (const CORBA::UserException& userex) {
    userex._tao_print_exception("ERROR: User Exception in "
      "TransportInterface::disassociate_participant!");
...
void
OpenDDS::DCPS::TransportInterface::disassociate_subscription(
  RepoId local_id,  // subscription
  RepoId remote_id) // publication
{
  DBG_ENTRY_LVL("TransportInterface","disassociate_subscription",6);

  DDS::DomainId_t domain_id(get_domain_id());
  RepoId participant_id(get_participant_id());

  if (DCPS_debug_level >= 4) {
    RepoIdConverter local_converter(local_id);
    RepoIdConverter remote_converter(remote_id);
    ACE_ERROR((LM_INFO,
               ACE_TEXT("(%P|%t) TransportInterface::disassociate_subscription:")
               ACE_TEXT(" disassociating local subscription %C from remote")
               ACE_TEXT(" publication %C.\n"),
               std::string(local_converter).c_str(),
               std::string(remote_converter).c_str()));
  }
  ...
  try {
    DCPSInfo_var repo = TheServiceParticipant->get_repository(domain_id);
    repo->disassociate_subscription(domain_id,
                                    participant_id,
                                    local_id,
                                    remote_id);
    ...
    userex._tao_print_exception("ERROR: User Exception in "
      "TransportInterface::disassociate_subscription!");
...
void
OpenDDS::DCPS::TransportInterface::disassociate_publication(
  RepoId local_id,  // publication
  RepoId remote_id) // subscription
{
  DBG_ENTRY_LVL("TransportInterface","disassociate_publication",6);
  ...
  if (DCPS_debug_level >= 4) {
    ...
    RepoIdConverter remote_converter(remote_id);
    ACE_DEBUG((LM_DEBUG,
               ACE_TEXT("(%P|%t) TransportInterface::disassociate_publication:")
               ACE_TEXT(" disassociating local publication %C from remote")
               ACE_TEXT(" subscription %C.\n"),
               std::string(local_converter).c_str(),
               std::string(remote_converter).c_str()));
  }
  ...
  try {
    DCPSInfo_var repo = TheServiceParticipant->get_repository(domain_id);
    repo->disassociate_publication(domain_id,
                                   participant_id,
                                   local_id,
                                   remote_id);
    ...
    userex._tao_print_exception("ERROR: User Exception in "
      "TransportInterface::disassociate_publication!");
