...
recevier(ACE_SOCK_Dgram& cli_dgram)
{
  ...
  std::vector<ACE_hrtime_t> latencies;
  ...
  while (rcv_cnt) {
    rcv_cnt = cli_dgram.recv (&data,
                              ...
                              &timeout);
    if (rcv_cnt > 0) {
      latencies.push_back(ACE_OS::gethrtime()-data.sending_time_);
    }
    else if (rcv_cnt < 0) {
      ACE_ERROR((LM_ERROR,
                 ACE_TEXT("(%P|%t) %p\n"),
  ...
  ACE_hrtime_t average = std::accumulate(latencies.begin(), latencies.end(), 0)/latencies.size();
  std::cout << "The average latency is " << average/1000 << " microseconds\n";
