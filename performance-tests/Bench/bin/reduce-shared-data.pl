...
reduce-shared-data.pl - reduce test results into plottable data
...
Individual data points for single hop latency from the input file.
...
TODO: Modify this for shared data reduction and extraction.
...
=head1 EXAMPLE
  ...
  reduce-shared-data.pl tcp/latency-1000.data > data/latency-tcp-1000.gpd
...
BEGIN {
  # Start input by skipping to the first Last Hop section.
  $skip = 1;
...
# Only process sections that are for the last hop.
$skip = 1 if /^ --- full path/;
$skip = 0 if /^ --- last hop/;
next if $skip;
...
# Value desired is for a single hop.
my $value = $_;
