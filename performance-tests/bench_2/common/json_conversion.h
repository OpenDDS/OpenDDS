...
template<typename IDL_Type, typename Writer_Type = rapidjson::Writer>
bool idl_2_json(const IDL_Type& idl_value, std::ostream& os,
    int max_decimal_places = Writer_Type<rapidjson::OStreamWrapper>::kDefaultMaxDecimalPlaces) {
  rapidjson::Document document;
  document.SetObject();
  ...
  if (max_decimal_places != Writer_Type<rapidjson::OStreamWrapper>::kDefaultMaxDecimalPlaces) {
