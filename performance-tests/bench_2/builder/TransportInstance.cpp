...
{
  Log::log() << "Checking for transport config '" << name_ << "'" << std::endl;
  config_ = OpenDDS::DCPS::TransportRegistry::instance()->get_config(config.name.in());
  if (!config_) {
    Log::log() << "Creating transport config '" << name_ << "'" << std::endl;
    config_ = OpenDDS::DCPS::TransportRegistry::instance()->create_config(config.name.in());
    Log::log() << "Checking for transport instance '" << name_ << "'" << std::endl;
    ...
    if (!inst_) {
      ...
      if (inst_) {
        Log::log() << "Creating transport instance '" << name_ << "' of type " << config.type << std::endl;
      } else {
    ...
    } else if (std::string(config.type.in()) == inst_->transport_type_) {
