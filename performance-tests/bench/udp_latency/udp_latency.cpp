...
static void receiver(ACE_SOCK_Dgram& cli_dgram)
{
  ...
  while (rcv_cnt) {
    rcv_cnt = cli_dgram.recv(&data, sizeof (data), peer_addr, 0, &timeout);
    if (rcv_cnt > 0) {
...
int echo_server(int port)
{
  ...
  while (rcv_cnt) {
    rcv_cnt = server_dgram.recv(&data, sizeof (data), peer_addr, 0, &timeout);
    if (rcv_cnt >= 0) {
      if (server_dgram.send(&data, static_cast<size_t>(rcv_cnt), peer_addr) < 0) {
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("(%P|%t) %p\n"),
  ...
  try {
    ...
    if (parse_args(argc, argv) == 0) {
      ...
      if (server_port) {
        ...
        if (ACE_Thread_Manager::instance()->spawn(ACE_THR_FUNC(sender), &cli_dgram, THR_NEW_LWP | THR_DETACHED) == -1) {
