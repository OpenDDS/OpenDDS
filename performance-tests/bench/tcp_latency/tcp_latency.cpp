...
static void receiver(ACE_SOCK_Stream& cli_stream)
{
  ...
  while (rcv_cnt) {
    rcv_cnt = cli_stream.recv_n(&data, message_size);
    if (rcv_cnt > 0) {
...
int echo_server(int port)
{
  ...
  ACE_INET_Addr echo_server_addr, cli_addr;
  echo_server_addr.set(static_cast<u_short>(port));
  ACE_SOCK_Stream server_stream;

  if (peer_acceptor.open(echo_server_addr) == -1) {
    ACE_ERROR((LM_ERROR,
               ACE_TEXT("(%P|%t) %p\n"),
  ...
  while (rcv_cnt) {
    rcv_cnt = server_stream.recv_n(&data, message_size);
    if (rcv_cnt > 0) {
      if (server_stream.send_n(&data, message_size) < 0) {
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("(%P|%t) %p\n"),
  ...
  while (ok && (c = getopt()) != -1) {
    switch (c) {
      ...
      case 'f':
        ...
        break;
      case 'm':
        message_size = static_cast<size_t>(ACE_OS::atoi(getopt.opt_arg()));
        ok = message_size != 0;
        break;
      case 's':
  ...
  try {
    ...
    if (parse_args(argc, argv) == 0) {
      ...
      if (server_port) {
        ...
        if (ACE_Thread_Manager::instance()->spawn(ACE_THR_FUNC(sender), &cli_stream, THR_NEW_LWP | THR_DETACHED) == -1) {
