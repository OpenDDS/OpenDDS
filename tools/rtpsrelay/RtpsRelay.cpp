...
int ACE_TMAIN(int argc, ACE_TCHAR* argv[])
{
  ...
  HandlerStatisticsReporter spdp_vertical_reporter(config, "VSPDP", handler_statistics_writer);
  SpdpHandler spdp_vertical_handler(config, "VSPDP", spdp_horizontal_addr, reactor, governor, association_table, responsible_relay_writer, responsible_relay_reader, rtps_discovery, crypto, spdp, spdp_vertical_reporter);
  HandlerStatisticsReporter sedp_vertical_reporter(config, "VSEDP", handler_statistics_writer);
  SedpHandler sedp_vertical_handler(config, "VSEDP", sedp_horizontal_addr, reactor, governor, association_table, responsible_relay_writer, responsible_relay_reader, rtps_discovery, crypto, sedp, sedp_vertical_reporter);
  HandlerStatisticsReporter data_vertical_reporter(config, "VDATA", handler_statistics_writer);
  DataHandler data_vertical_handler(config, "VDATA", data_horizontal_addr, reactor, governor, association_table, responsible_relay_writer, responsible_relay_reader, rtps_discovery, crypto, data_vertical_reporter);

  HandlerStatisticsReporter spdp_horizontal_reporter(config, "HSPDP", handler_statistics_writer);
  HorizontalHandler spdp_horizontal_handler(config, "HSPDP", reactor, governor, spdp_horizontal_reporter);
  HandlerStatisticsReporter sedp_horizontal_reporter(config, "HSEDP", handler_statistics_writer);
  HorizontalHandler sedp_horizontal_handler(config, "HSEDP", reactor, governor, sedp_horizontal_reporter);
  HandlerStatisticsReporter data_horizontal_reporter(config, "HDATA", handler_statistics_writer);
  HorizontalHandler data_horizontal_handler(config, "HDATA", reactor, governor, data_horizontal_reporter);
