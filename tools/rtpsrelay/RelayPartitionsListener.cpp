...
void RelayPartitionsListener::on_data_available(DDS::DataReader_ptr reader)
{
  ...
  for (CORBA::ULong idx = 0; idx != infos.length(); ++idx) {
    ...
    case DDS::ALIVE_INSTANCE_STATE:
      if (info.valid_data) {
        relay_partition_table_.complete_insert(guid_to_repoid(data.application_participant_guid()),
                                               data.partitions());
      }
      break;
    case DDS::NOT_ALIVE_DISPOSED_INSTANCE_STATE:
    case DDS::NOT_ALIVE_NO_WRITERS_INSTANCE_STATE:
      relay_partition_table_.complete_remove(guid_to_repoid(data.application_participant_guid()));
      break;
    }
  }
}
