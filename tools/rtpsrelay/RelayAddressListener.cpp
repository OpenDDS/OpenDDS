...
void RelayAddressListener::on_data_available(DDS::DataReader_ptr reader)
{
  ...
  for (CORBA::ULong idx = 0; idx != infos.length(); ++idx) {
    ...
    case DDS::ALIVE_INSTANCE_STATE:
      {
        ...
        relay_partition_table_.insert(guid_to_repoid(d.application_participant_guid()),
                                      d.name(),
                                      ACE_INET_Addr(d.address().c_str()));
      }
      break;
    ...
    case DDS::NOT_ALIVE_NO_WRITERS_INSTANCE_STATE:
      {
        const auto& d = data[idx];
        relay_partition_table_.remove(guid_to_repoid(d.application_participant_guid()),
                                      d.name());
      }
      break;
    }
