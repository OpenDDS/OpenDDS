...
struct Topic : public virtual LocalObject<DDS::DataReaderListener> {
  ...
  Topic(const std::string& a_name, const std::string& a_type_name, DDS::GuardCondition_var a_gc)
  : name(a_name)
...
struct DynamicTopic : public Topic {
  ...
  DynamicTopic(const std::string& a_name, const std::string& a_type_name, DDS::GuardCondition_var a_gc)
  : Topic(a_name, a_type_name, a_gc)
  {
  }
    ...
    DDS::PublicationBuiltinTopicData& pb)
  {
    ...
    topic = dp->create_topic(name.c_str(), tn, TOPIC_QOS_DEFAULT, 0, DEFAULT_STATUS_MASK);
    if (!topic) {
  ...
  try {
    ...
    while (true) {
      ...
      for (unsigned i = 0; i < pub_bit_seq.length(); ++i) {
        ...
        const std::string tn = pb.type_name.in();
        if (topic) {
          continue;
        }
        ACE_DEBUG((LM_DEBUG, "Learned about topic \"%C\" type \"%C\"\n",
          name.c_str(), tn.c_str()));
        topic = new DynamicTopic(name, tn, gc);
