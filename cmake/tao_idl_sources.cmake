...
function(_opendds_tao_append_runtime_lib_dir_to_path dst)
  if(MSVC)
    ...
    set(val "${val}${TAO_LIB_DIR};$<TARGET_FILE_DIR:OpenDDS::Util>")
  else()
    ...
    string(REPLACE "\\" "/" tmp "${TAO_LIB_DIR}:${OPENDDS_LIB_DIR}")
    set(val "\"${val}${tmp}\"")
  endif()
...
function(_opendds_ensure_generated_output_dir target include_base file o_arg output_dir_var)
  ...
  elseif(include_base AND NOT OPENDDS_FILENAME_ONLY_INCLUDES)
    ...
    if(rel_file MATCHES "^\\.\\.")
      message(WARNING "THIS IS WRONG: ${rel_file}")
    endif()
    if(rel_dir)
...
function(_opendds_get_generated_idl_output
    target include_base idl_file o_arg output_prefix_var output_dir_var)
  _opendds_ensure_generated_output_dir(${target} "${include_base}" "${idl_file}" "${o_arg}" output_dir)
...
function(_opendds_tao_idl target)
  ...
  foreach(idl_file ${arg_IDL_FILES})
    ...
    if(idl_cmd_arg_-oS)
      _opendds_get_generated_idl_output(
        ${target} "${arg_INCLUDE_BASE}" ${idl_file} "${idl_cmd_arg_-oS}" skel_output_prefix skel_output_dir)
    ...
    if(idl_cmd_arg_-oA)
      _opendds_get_generated_idl_output(
        ${target} "${arg_INCLUDE_BASE}" "${idl_file}" "${idl_cmd_arg_-oA}" anyop_output_prefix anyop_output_dir)
