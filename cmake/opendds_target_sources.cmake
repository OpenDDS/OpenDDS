...
function(_opendds_get_sources_and_options
  ...
  if(arg_CUSTOM_EXPORT)
    list(LENGTH arg_CUSTOM_EXPORT custom_export_len)
    if(NOT custom_export_len EQUAL 2)
      message(FATAL_ERROR
        "The opendds_target_sources CUSTOM_EXPORT option takes a header path and macro name, "
        "but was passed ${custom_export_len} values")
    endif()
    set(${custom_export} "${arg_CUSTOM_EXPORT}" PARENT_SCOPE)
  else()
...
function(opendds_target_sources target)
  ...
  if(NOT custom_export)
    ...
    set(use_export "${custom_export}")
    # Assuming the custom export header is compatible with ones generated by generate_export_file.pl
    list(GET custom_export 1 custom_macro)
    string(FIND ${custom_macro} "_Export" index)
    string(SUBSTRING ${custom_macro} 0 ${index} export_prefix)
    string(TOUPPER ${export_prefix} macro_prefix)
    if(target_type STREQUAL "SHARED_LIBRARY")
