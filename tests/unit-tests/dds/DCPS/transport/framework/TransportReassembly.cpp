...
void test_fill_ltor()
{
  ...
  TEST_ASSERT(6 == base);
  TEST_ASSERT(256 == gaps.result_bits);
  TEST_ASSERT(gaps.check_gap(7));    // Gap
  TEST_ASSERT(gaps.check_gap(8));    // Gap
}
...
int ACE_TMAIN(int, ACE_TCHAR*[])
{
  ...
  {
    ex._tao_print_exception("Exception caught in TransportReassembly.cpp:");
    return 1;
  }
  return 0;
}
