    ...
    for (OpenDDS::XTypes::DynamicTypeMembersById::iterator iter = dtmbi.begin(); iter != dtmbi.end(); ++iter)
    {
      ...
      if (member_tk == OpenDDS::XTypes::TK_INT32) {
        ...
        if (ele_type == OpenDDS::XTypes::TK_ENUM) {
          OpenDDS::XTypes::DynamicData::Int32Seq my_int_seq;
        ...
        if (aliased_type == OpenDDS::XTypes::TK_ARRAY) {
          ...
          if (ele_type == OpenDDS::XTypes::TK_BOOLEAN) {
            OpenDDS::XTypes::DynamicData::BooleanSeq my_bool_seq;
            ...
            if (*my_bool_seq.begin() != true || *my_bool_seq.end() != false) {
              ACE_ERROR_RETURN((LM_ERROR, "(%P|%t) Error: print_dynamic_data - bool_seq: <%d,%d> does not match expected values: <1,0>\n",
                                *my_bool_seq.begin(), *my_bool_seq.end()), -1);
            }
            std::cout << "  " << type_name << " " <<  member_name << " = ";
            std::cout << " < " << std::to_string(*my_bool_seq.begin()) << ", ";
            std::cout << std::to_string(*my_bool_seq.end()) << ">\n";
          }
        }
...
public:
                                       ...
                                       const OpenDDS::DCPS::RawDataSample& sample)
  {
    ...
    ACE_DEBUG((LM_DEBUG,
               ACE_TEXT("TestRecorderListener::on_sample_data_received\n")));
    sleep(3);
                                   ...
                                   const ::DDS::SubscriptionMatchedStatus& status )
  {
    if (status.current_count == 1) {
        ACE_DEBUG((LM_DEBUG,
                 ACE_TEXT("TestRecorderListener -- a writer connect to recorder\n")));
    }
    else if (status.current_count == 0 && status.total_count > 0) {
      ACE_DEBUG((LM_DEBUG,
                 ACE_TEXT("TestRecorderListener -- writer disconnect with recorder\n")));
      sem_.release();
    }
  }
