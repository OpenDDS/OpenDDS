...
int ACE_TMAIN(int argc, ACE_TCHAR *argv[])
{
  ...
  if (Utils::wait_match(dw_automatic, 1, Utils::EQ)) {
    cerr << "Error waiting for match for dw_automatic" << std::endl;
    return 1;
  }

  if (Utils::wait_match(automatic_dr, 1, Utils::EQ)) {
    cerr << "Error waiting for match for automatic_dr" << std::endl;
    return 1;
  }

  if (Utils::wait_match(dw_manual, 2, Utils::EQ)) {
    cerr << "Error waiting for match for dw_manual" << std::endl;
    return 1;
  }

  if (Utils::wait_match(remote_manual_dr, 1, Utils::EQ)) {
    cerr << "Error waiting for match for remote_manual_dr" << std::endl;
    return 1;
  }

  if (Utils::wait_match(local_manual_dr, 1, Utils::EQ)) {
    cerr << "Error waiting for match for local_manual_dr" << std::endl;
    return 1;
  }
  ...
  } else if (remote_manual_drl_servant->no_writers_generation_count() != num_unlively_periods - 1) {
    status = 1;
    ACE_ERROR((LM_ERROR,
  ...
  } else if (local_manual_drl_servant->no_writers_generation_count() != num_unlively_periods - 1) {
    status = 1;
    ACE_ERROR((LM_ERROR,
