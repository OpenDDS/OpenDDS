#include "OldDeviceTypeSupportImpl.h"
...
int ACE_TMAIN(int argc, ACE_TCHAR* argv[])
{
  ...
  participant_qos.user_data.value.length(sizeof HelloWorld::OLDDEV);
  std::memcpy(participant_qos.user_data.value.get_buffer(), HelloWorld::OLDDEV, sizeof HelloWorld::OLDDEV);
  ...
  distributed_condition_set->wait_for(HelloWorld::OLDDEV, HelloWorld::CONTROLLER, HelloWorld::CONTROLLER_READY);

  HelloWorld::Message message;
  message.deviceId = HelloWorld::OLDDEV;
  message.st = HelloWorld::Complete;
  ...
  while (!done) {
    ...
    for (unsigned int idx = 0; idx != messages.length(); ++idx) {
      if (infos[idx].valid_data && 0 == std::strcmp(messages[idx].deviceId.in(), HelloWorld::OLDDEV)) {
        ACE_DEBUG((LM_DEBUG, "received %C %d\n", messages[idx].deviceId.in(), messages[idx].st));
        distributed_condition_set->post(HelloWorld::OLDDEV, HelloWorld::OLDDEV_DONE);
        done = true;
        break;
      }
    }
