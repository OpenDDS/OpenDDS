...
static OpenDDS::DCPS::String actor;
...
int parse_args(int argc, ACE_TCHAR *argv[])
{
  ...
  while (arg_shifter.is_anything_left())
  {
    ...
    {
      actor = ACE_TEXT_ALWAYS_CHAR(currentArg);
      arg_shifter.consume_arg();
    }
    else
    {
...
int ACE_TMAIN(int argc, ACE_TCHAR *argv[])
{
  ...
  try
  {
    ...
    const OpenDDS::DCPS::String actor1 = OpenDDS::DCPS::String(MY_TOPIC1) + " " + actor;
    const OpenDDS::DCPS::String actor2 = OpenDDS::DCPS::String(MY_TOPIC2) + " " + actor;
    const OpenDDS::DCPS::String actor3 = OpenDDS::DCPS::String(MY_TOPIC3) + " " + actor;
    const OpenDDS::DCPS::String actor4 = OpenDDS::DCPS::String(MY_TOPIC4) + " " + actor;

    ::DDS::DataReaderListener_var drl1 =
        new DataReaderListenerImpl<T1::Foo1>(dcs, actor1, print1);
    ::DDS::DataReaderListener_var drl2 =
        new DataReaderListenerImpl<T2::Foo2>(dcs, actor2, print2);
    ::DDS::DataReaderListener_var drl3 =
        new DataReaderListenerImpl<T3::Foo3>(dcs, actor3, print3);
    ::DDS::DataReaderListener_var drl4 =
        new DataReaderListenerImpl<T3::Foo3>(dcs, actor4, print3);
    ...
    if (topics & TOPIC_T1)
    {
      ...
      Utils::wait_match(dr1, 1, Utils::EQ);
      dcs->post(actor1, "ready");
    }
    ...
    if (topics & TOPIC_T4)
    {
      ...
      Utils::wait_match(dr1, 1, Utils::EQ);
      dcs->post(actor4, "ready");
    }

    if (topics & TOPIC_T1)
    {
      dcs->wait_for(actor1, actor1, "done");
    }
    if (topics & TOPIC_T2)
    {
      dcs->wait_for(actor2, actor2, "done");
    }
    if (topics & TOPIC_T3)
    {
      dcs->wait_for(actor3, actor3, "done");
    }
    if (topics & TOPIC_T4)
    {
      dcs->wait_for(actor4, actor4, "done");
    }
