...
private:
  ...
  bool check_status(const DDS::InstanceHandle_t& i1r1, const DDS::InstanceHandle_t& i1r2,
                    ...
                    const CORBA::Long expected, CORBA::Long& delta2);
  ...
  bool check_change(DDS::RequestedDeadlineMissedStatus& s1, DDS::RequestedDeadlineMissedStatus& s2, CORBA::Long& delta2) const;
...
Subscriber::Subscriber(int argc, ACE_TCHAR* argv[]) : domain_(argc, argv, "Subscriber")
{
  ...
  try {
    ...
    ACE_DEBUG((LM_INFO, ACE_TEXT("(%P|%t) DataReaderQos deadline.period (%d sec)\n"), qos.deadline.period.sec));
    ACE_DEBUG((LM_INFO, ACE_TEXT("(%P|%t) create 2 readers and dr1_ has a listener\n")));
    // Reader1/Reader2 test deadline expiration handling code with/without listener callback
    dr1_ = sub_->create_datareader(domain_.topic().in(), qos, listener_.in(), OpenDDS::DCPS::DEFAULT_STATUS_MASK);
    dr2_ = sub_->create_datareader(domain_.topic().in(), qos, 0, OpenDDS::DCPS::DEFAULT_STATUS_MASK);
    if (!dr1_ || !dr2_) {
...
int Subscriber::run()
{
  ...
  CORBA::Long delta2 = 0;
  if (!check_status(ins1r1, ins1r2, ins2r1, ins2r2, Domain::N_Instance, delta2)) {
    return 1;
  }
  ...
  if (!check_status(ins1r1, ins1r2, ins2r1, ins2r2, 3 * Domain::N_Instance, delta2)) {
    return 1;
  }
...
bool Subscriber::check_change(DDS::RequestedDeadlineMissedStatus& s1, DDS::RequestedDeadlineMissedStatus& s2, CORBA::Long& delta2) const
{
  ACE_DEBUG((LM_INFO, ACE_TEXT("(%P|%t) Subscriber::check_change\n")));
  // check if the total count changed compared to the last time we got it through dr1's listener;
  // dr2 has no listener, so the total_count_change should match the total count
  CORBA::Long delta1 = s1.total_count - listener_i_->requested_deadline_total_count();
  delta2 = s2.total_count - delta2; // also save for the next check
