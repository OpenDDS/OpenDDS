...
if ($#ARGV > 0) {
    print "Usage: run_test.pl <num_writers>\n";
    print "       <num_writers> is optional and defaults to 4\n";
    exit -1;
}
if ($#ARGV == 0) {
    $num_writers = $ARGV[0];
...
my $DCPSREPO=PerlDDS::create_process ("$ENV{DDS_ROOT}/bin/DCPSInfoRepo",
                                      " -DCPSConfigFile single.ini $repo_bit_conf -o $dcpsrepo_ior");
print $DCPSREPO->CommandLine(), "\n";

unlink("pub.log");

my $publisher=PerlDDS::create_process ("publisher",
                                       " -DCPSConfigFile single.ini -DCPSDebugLevel 5 -DCPSTransportDebugLevel 5 "
                                       . " -ORBLogFile pub.log -ORBVerboseLogging 1 $app_bit_conf $cfgfile"
...
for (my $i = 0; $i < $num_readers; $i++) {
  my $subscriber=PerlDDS::create_process ("subscriber",
                                          ...
                                          . " -DCPSConfigFile single.ini -DCPSInfoRepo file://$dcpsrepo_ior"
                                          . " -n $num_writes"
