...
TEST(RapidJsonTest, ParseTest)
{
  ...
  ASSERT_EQ(sample.seq1.length(), 7);
  ...
  ASSERT_EQ(sample.seq2.length(), 3);
  ASSERT_EQ(sample.seq2[0], 42);
  ASSERT_EQ(sample.seq2[1], 777);
  ASSERT_EQ(sample.seq2[2], 123454321);
  ASSERT_EQ(sample.ns.length(), 2);
  ASSERT_EQ(sample.ns[0].length(), 3);
  ...
  ASSERT_EQ(sample.ns[1].length(), 4);
  ...
  ASSERT_EQ(sample.mu._d(), Mod::three);
  ASSERT_EQ(sample.mu.d(), 5678);
...
TEST(RapidJsonTest, SerializeTest)
{
  ...
  ASSERT_EQ(val["bt"]["us"], 32777);
  ASSERT_EQ(val["bt"]["s"], -32765);
  ASSERT_EQ(val["bt"]["ul"], 2147483690);
