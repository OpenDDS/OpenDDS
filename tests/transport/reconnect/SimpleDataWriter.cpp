...
{
  return (this->num_messages_delivered_ == this->num_messages_) ? 1 : 0;
}
...
DDS_TEST::run(int num_messages, int msg_size)
{
  ...
  this->num_messages_sent_      = 0;
  this->num_messages_           = num_messages;

  int num_samples_per_message = 1;

  OpenDDS::DCPS::DataSampleElementAllocator* allocator =
    new OpenDDS::DCPS::DataSampleElementAllocator(num_samples_per_message);
  //Cleanup cleanup(allocator, samples);
  OpenDDS::DCPS::TransportSendElementAllocator* trans_allocator =
    new OpenDDS::DCPS::TransportSendElementAllocator(num_samples_per_message, sizeof (OpenDDS::DCPS::TransportSendElement));

  int n = 0;
  while (num_messages_delivered_ < num_messages) {
    n++;

    this->num_messages_sent_++;
    ...
    for (int i = 1; i <= num_samples_per_message; ++i) {
    ...
    ACE_DEBUG((LM_INFO,
               "(%P|%t) Publisher send time required was %d.%d seconds.\n",
               total.sec(),
               total.usec() % 1000000));
    ACE_OS::sleep(1);
  }
