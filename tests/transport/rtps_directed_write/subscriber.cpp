...
public:
    ...
    : config(ac), index(readerIndex), done_(false), seq_()
  {
    ...
    enable_transport(config.readersReliable(), false, get_guid()); // (reliable, durable, guid)
    ...
    std::cerr << "Reader " << LogGuid(get_guid()).conv_ << " called associate()\n";
  }
  ...
  GUID_t get_guid() const { return config.getSubRdrId(index); }
...
void SimpleDataReader::data_received(const ReceivedDataSample& sample)
{
  ...
  oss << "data_received() by " << LogGuid(get_guid()).c_str() << "\n\t"
