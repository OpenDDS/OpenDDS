...
SubDriver::parse_arg_p(const char* arg, bool& flag)
{
  ...
  guid.entityId.entityKey[2] = ACE_OS::atoi(pub_id_str.c_str());

  ACE_INET_Addr pub_addr(pub_addr_str.c_str());

  subscriber_.add_remote_publisher(guid, pub_addr, pub_addr_str);
...
SubDriver::parse_arg_s(const char* arg, bool& flag)
{
  ...
  guid.guidPrefix[11] = 1; // participantId = 1
  guid.entityId.entityKind = OpenDDS::DCPS::ENTITYKIND_USER_WRITER_WITH_KEY;
  guid.entityId.entityKey[2] = ACE_OS::atoi(sub_id_str.c_str());

  this->local_address_ = ACE_INET_Addr(this->sub_addr_str_.c_str());

  subscriber_.set_local_subscriber(guid);
