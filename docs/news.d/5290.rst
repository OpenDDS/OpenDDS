...
- Hardened XCDR1 and XCDR2 deserialization against malformed or inconsistent wire data that could otherwise cause excessive allocation, out-of-bounds reads, or undefined behavior:
  ...
  - XCDR2 ``DHEADER`` sizes, and XCDR1 and XCDR2 mutable member sizes, are enforced as read boundaries, so a delimited object or member that under-reports its size is rejected instead of reading into later data (:ghissue:`5287`).
