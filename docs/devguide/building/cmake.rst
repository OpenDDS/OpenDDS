...
.. cmake:func:: opendds_target_sources
  ...
  .. cmake:func:arg:: USE_EXPORT <export-header> <export-macro>
    ...
    If there are multiple calls to ``opendds_target_sources`` or ``opendds_export_header`` on the same target, only the export header settings of the first call are used.
