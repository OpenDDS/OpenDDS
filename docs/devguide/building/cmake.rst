...
Arguments can be passed to ``find_package(OpenDDS COMPONENTS <argument>...)`` (or alternatively ``find_package(OpenDDS REQUIRED [COMPONENTS] <argument>...)``) to add to what's required.
These can be:
...
.. cmake:func:: opendds_target_sources
  ...
  .. cmake:func:arg:: USE_EXPORT <export-header> <export-macro>
    ...
    If more control is needed, use :cmake:func:`opendds_export_header(EXISTING)` before ``opendds_target_sources``.
...
.. cmake:func:: opendds_export_header
  ...
  ``opendds_export_header`` is called automatically from :cmake:func:`opendds_target_sources` unless one of the functions was already called and generated headers are treated like other :ref:`generated files <cmake-files-props>`.

  The generated export headers are used by IDL-generated code, but can also be used your own code.
  ...
  .. cmake:func:arg:: INCLUDE <name>

    Set the name of the export header to be generated (if not using :cmake:func:`opendds_export_header(EXISTING)`) and used by the IDL-generated code.
  ...
  .. cmake:func:arg:: HEADER_MACRO <name>
    ...
    The name of the macro used to signal that symbols should be exported.
    By default this is ``<uppercase-macro-prefix-name>_HAS_DLL``.

  .. cmake:func:arg:: SOURCE_MACRO <name>

    The name of the macro used to signal that a shared library is being built.
    By default this is ``<uppercase-macro-prefix-name>_BUILD_DLL``.

  .. cmake:func:arg:: FORCE_EXPORT TRUE|FALSE

    By default symbols are exported based on if the target library is shared or static.
    If this defined, this overrides that decision.
