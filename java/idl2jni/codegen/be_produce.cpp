                 ...
                 BE_JNIGlobalData::stream_enum_t which)
{
  ...
  if (which == BE_JNIGlobalData::STUB_H || which == BE_JNIGlobalData::SKEL_H) {
    macrofied = to_macro(fn);
    out << "/* -*- C++ -*- */\n";
    out << "#ifndef " << macrofied << "\n#define " << macrofied << '\n';
    string taoheader = to_tao(fn);
    out << "#include \"" << be_jni_global->tao_inc_pre_ << taoheader << "\"\n";

  } else if (which == BE_JNIGlobalData::STUB_CPP ||
             which == BE_JNIGlobalData::SKEL_CPP) {
    string header = to_header(fn);
    out << "#include \"" << header
    << "\"\n#include \"idl2jni_runtime.h\"\n\n";
  }

  out << be_jni_global->get_include_block(which);

  if (which == BE_JNIGlobalData::SKEL_H) {
...
BE_JNIInterface::produce()
{
  ...
  while (idl) {
    ...
    if (0 == strncmp("#include", buffer, 8)) { //FUTURE: account for comments?
      const string inc(buffer + 8);
      const size_t delim1 = inc.find_first_of("<\"");
      const size_t delim2 = inc.find_first_of(">\"", delim1 + 1);
      const string included(inc, delim1 + 1, delim2 - delim1 - 1);
      const size_t len = included.size();
      string base_name;
      ...
      be_jni_global->add_include(stb_inc.c_str());

      if (be_jni_global->do_server_side()) {
        string skl_inc = base_name + "JS.h";
        be_jni_global->add_include(skl_inc.c_str(),
                               BE_JNIGlobalData::SKEL_CPP);
      }
    }
  }
  ...
  be_jni_global->open_streams(idl_fn);
  ...
  be_jni_global->multicast("/* Generated by ");
  be_jni_global->multicast(idl_global->prog_name());
  be_jni_global->multicast(" running on input file ");
  be_jni_global->multicast(idl_global->filename()->get_string());
  ...
  postprocess(be_jni_global->stub_header_name_.c_str(),
              be_jni_global->stub_header_, BE_JNIGlobalData::STUB_H);
  postprocess(be_jni_global->stub_impl_name_.c_str(),
              be_jni_global->stub_impl_, BE_JNIGlobalData::STUB_CPP);

  if (be_jni_global->do_server_side()) {
    postprocess(be_jni_global->skel_header_name_.c_str(),
                be_jni_global->skel_header_, BE_JNIGlobalData::SKEL_H);
    postprocess(be_jni_global->skel_impl_name_.c_str(),
                be_jni_global->skel_impl_, BE_JNIGlobalData::SKEL_CPP);
  }

  be_jni_global->close_streams();
}
