...
namespace SafetyProfile {
  ...
  template <>
  struct StringTraits<CORBA::Char>
  {
    static CORBA::Char* alloc(CORBA::ULong len) { return CORBA::string_alloc(len); }
    static CORBA::Char* empty() { return dup(""); }
    static CORBA::Char* dup(const CORBA::Char* s) { return CORBA::string_dup(s); }
    static void free(CORBA::Char* s) { if (s) CORBA::string_free(s); }
  ...
  typedef CORBA::ULong seq_size_type;
  typedef CORBA::Boolean seq_flag_type;
