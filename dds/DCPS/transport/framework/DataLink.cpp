...
DataLink::handle_exception(ACE_HANDLE /* fd */)
{
  if(this->scheduled_to_stop_at_ == ACE_Time_Value::zero) {
    ...
    return 0;
  } else if (this->scheduled_to_stop_at_ <= monotonic_time()) {
    ...
    ACE_Reactor_Timer_Interface* reactor = impl_.timer();
    ACE_Time_Value future_release_time = this->scheduled_to_stop_at_ - monotonic_time();
...
DataLink::schedule_delayed_release()
{
  ...
  ACE_Time_Value future_release_time = monotonic_time() + this->datalink_release_delay_;
