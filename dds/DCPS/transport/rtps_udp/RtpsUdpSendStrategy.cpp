                                    ...
                                    RTPS::SubmessageSeq& submessages)
{
  const DDS::Security::DatawriterCryptoHandle writer_crypto_handle =
    link_->security_config()->get_handle_registry()->get_local_datawriter_crypto_handle(pub_id);
                                              ...
                                              CORBA::Octet msgId)
{
  ...
  if (std::memcmp(&GUID_UNKNOWN, &receiver, sizeof receiver)) {
    DatareaderCryptoHandle drch =
      link_->security_config()->get_handle_registry()->get_remote_datareader_crypto_handle(receiver);
                                              ...
                                              CORBA::Octet msgId)
{
  ...
  if (std::memcmp(&GUID_UNKNOWN, &receiver, sizeof receiver)) {
    DatawriterCryptoHandle dwch = link_->security_config()->get_handle_registry()->get_remote_datawriter_crypto_handle(receiver);
  ...
  while (ok && parser.remaining()) {
    ...
    case RTPS::HEARTBEAT_FRAG: {
      ...
      if (!encode_writer_submessage(receiver, replacements, crypto, plainSm,
                                    link_->security_config()->get_handle_registry()->get_local_datawriter_crypto_handle(sender), submessage_start, smhdr.submessageId)) {
        ok = false;
      }
      break;
    }
    ...
    case RTPS::NACK_FRAG: {
      ...
      if (!encode_reader_submessage(receiver, replacements, crypto, plainSm,
                                    link_->security_config()->get_handle_registry()->get_local_datareader_crypto_handle(sender), submessage_start, smhdr.submessageId)) {
        ok = false;
      }
      break;
    }
