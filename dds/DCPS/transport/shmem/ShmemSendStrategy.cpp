...
ShmemSendStrategy::start_i()
{
  ...
  ShmemAllocator* shmem_allocator = link_->local_allocator();

  const size_t n_elems = datalink_control_size_ / sizeof(ShmemData),
    extra = datalink_control_size_ % sizeof(ShmemData);

  void* mem = 0;
  if (shmem_allocator == 0 || (mem = shmem_allocator->calloc(datalink_control_size_)) == 0) {
  ...
  shmem_allocator->bind(bound_name_.c_str(), mem);

  {
    ShmemDataLink::PeerAllocatorProxy proxy(*link_);
    ShmemAllocator* shmem_allocator = proxy.peer_allocator();
    shmem_allocator->find("Semaphore", mem);
  }
...
ShmemSendStrategy::send_bytes_i(const iovec iov[], int n)
{
  ...
  ShmemAllocator* shmem_allocator = link_->local_allocator();
  void* from_pool = 0;
  if (shmem_allocator == 0 || (from_pool = shmem_allocator->malloc(pool_alloc_size)) == 0) {
    VDBG_LVL((LM_ERROR, "(%P|%t) ERROR: ShmemSendStrategy for link %@ failed "
  ...
  void* mem = 0;
  if (-1 == shmem_allocator->find(bound_name_.c_str(), mem) || mem == 0) {
    VDBG_LVL((LM_ERROR, "(%P|%t) ERROR: ShmemSendStrategy for link %@ failed "
  ...
  for (ShmemData* it = reinterpret_cast<ShmemData*>(mem);
    ...
    if (it->status_ == ShmemData::RecvDone) {
      shmem_allocator->free(it->payload_);
