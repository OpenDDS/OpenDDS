...
ShmemSendStrategy::start_i()
{
  ...
  ShmemAllocator* allocator = link_->local_allocator();

  const size_t n_elems = datalink_control_size_ / sizeof(ShmemData),
    extra = datalink_control_size_ % sizeof(ShmemData);

  void* mem = 0;
  if (allocator == 0 || (mem = allocator->calloc(datalink_control_size_)) == 0) {
  ...
  allocator->bind(bound_name_.c_str(), mem);

  {
    ShmemDataLink::PeerAllocatorProxy proxy(*link_);
    ShmemAllocator* allocator = proxy.peer_allocator();
    allocator->find("Semaphore", mem);
  }
...
ShmemSendStrategy::send_bytes_i(const iovec iov[], int n)
{
  ...
  ShmemAllocator* allocator = link_->local_allocator();
  void* from_pool = 0;
  if (allocator == 0 || (from_pool = allocator->malloc(pool_alloc_size)) == 0) {
    VDBG_LVL((LM_ERROR, "(%P|%t) ERROR: ShmemSendStrategy for link %@ failed "
  ...
  void* mem = 0;
  if (-1 == allocator->find(bound_name_.c_str(), mem) || mem == 0) {
    VDBG_LVL((LM_ERROR, "(%P|%t) ERROR: ShmemSendStrategy for link %@ failed "
