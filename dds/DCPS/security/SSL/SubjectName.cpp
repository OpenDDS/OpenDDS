...
int
SubjectName::parse(const std::string& in, bool permissive)
{
  return parse(in.data(), permissive);
}

int
SubjectName::parse(const char* in, bool permissive)
{
  if (in == NULL) {
    return -1;
  }
...
int
SubjectName::simple_avp_seq_parse(const char* in, const char* s_del, const char* a_del, const char* s_trim, const char* a_trim, bool push_back)
{
  std::string input(in);
  ...
  // This parser is meant only to cover basic cases, more advanced cases will need something different
  // Specifically, this won't correctly handle all escaped characters or all UTF-8 strings
...
int
SubjectName::parse_permissive(const char* in)
{
  return simple_avp_seq_parse(in, ",/", "=", " ", " ", false);
}

int
SubjectName::parse_dce(const char* in)
{
  return simple_avp_seq_parse(in, "/", "=", " ", " ", true);
}

int
SubjectName::parse_ldap_v3(const char* in)
{
  return simple_avp_seq_parse(in, ",", "=", " ", " ", false);
}

bool
SubjectName::operator==(const SubjectName& rhs) const
{
  bool result = (map_.size() == rhs.map_.size());
  for (AttrMap::const_iterator i1 = map_.begin(), i2 = rhs.map_.begin(); result == true && i1 != map_.end() && i2 != rhs.map_.end(); ++i1, ++i2) {
    if (i1->first.compare(i2->first) != 0 || i1->second.compare(i2->second) != 0) {
      result = false;
    }
  }
  return result;
