  ...
  if (!state_.empty()) {
    ...
    if (enclosing_ek == DDS::FINAL || enclosing_ek == DDS::APPENDABLE) {
      if (extensibility == DDS::FINAL) {
        ...
        // the size is required for a Dheader or a Emheader.
        // So we don't need to cache the size of this struct.
        // The concrete write_* methods will add the size of for the concrete members of this struct.
        Metadata metadata(extensibility);
        // Copy so that the total size can be built up continuously.
        metadata.total_size = state_.top().total_size;
        state_.push(metadata);
        return;
      } else {
        // This struct is either APPENDABLE or MUTABLE. Thus, it needs
        // its own entry in state_ and its size needs to be cached. Since we know the
        // the first thing in this struct is Dheader, we can perform alignment here.
...
void SerializedSizeValueWriter::end_struct()
{
  ...
  if (!state_.empty()) {
    ...
    if (enclosing_ek == DDS::FINAL || enclosing_ek == DDS::APPENDABLE) {
      if (extensibility == DDS::FINAL) {
        // Copy the accumulated size back to the containing entry.
        state_.top().total_size = total_size;
        return;
      } else {
        // This struct is APPENDABLE or MUTABLE.
        // Here the total size of this struct is ready, now update the total size of
