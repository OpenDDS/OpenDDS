...
=======
  ...
  OpenDDS::DCPS::ThreadStatus* status = TheServiceParticipant->get_thread_statuses();

  ACE_Time_Value expire(0);
  ...
  while (!shutting_down_) {
    for (Msg* msg = 0; getq(msg, &expire) != -1; /*no increment*/) {
      if (DCPS::DCPS_debug_level > 5) {
        ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) Sedp::Task::svc ") ACE_TEXT("got message from queue type %C\n"),
                   msg->msgTypeToString()));
      }

      // update thread status report
      ...
      if (expire > 0 && now > expire) {
        ...
        if (status) {
          if (DCPS::DCPS_debug_level > 4) {
            ACE_DEBUG((LM_DEBUG,
                       "%T (%P|%t) Sedp::Task::svc. Updating thread status.\n"));
          }
      ...
      switch (msg->type_) {
        ...
        case Msg::MSG_STOP:
          ...
          while (!msg_queue_->is_empty() && getq(msg, &expire) != -1) {
            delete msg;
            // update thread status report
            ACE_Time_Value now = MonotonicTimePoint::now().value();
            if (expire > 0 && now > expire) {
              ...
              if (status) {
                if (DCPS::DCPS_debug_level > 4) {
                  ACE_DEBUG((LM_DEBUG,
                             "%T (%P|%t) Sedp::Task::svc MSG_STOP: Updating thread status.\n"));
                }
                status->lock.acquire_write();
                status->map[tid] = now;
                status->lock.release();
