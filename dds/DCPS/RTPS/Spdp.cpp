...
void Spdp::SpdpTransport::process_relay_sra(ICE::ServerReflexiveStateMachine::StateChange sc)
{
  ...
  case ICE::ServerReflexiveStateMachine::SRSM_Change:
    ...
    outer->sedp_->job_queue()->enqueue(DCPS::make_rch<DCPS::WriteConnectionRecords>(outer->bit_subscriber_.value(), true, connection_record));
    break;
  case ICE::ServerReflexiveStateMachine::SRSM_Unset:
    ...
    outer->sedp_->job_queue()->enqueue(DCPS::make_rch<DCPS::WriteConnectionRecords>(outer->bit_subscriber_.value(), false, connection_record));
    break;
  }
#else
  ACE_UNUSED_ARG(sc);
