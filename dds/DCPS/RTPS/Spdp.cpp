...
Spdp::handle_handshake_message(const DDS::Security::ParticipantStatelessMessage& msg)
{
  ...
  if ((iter->second.auth_state_ == DCPS::AS_HANDSHAKE_REPLY || iter->second.auth_state_ == DCPS::AS_HANDSHAKE_REPLY_SENT) && msg.related_message_identity.source_guid == GUID_UNKNOWN) {
    ...
    DCPS::Serializer out_serializer(
      &temp_buff, Encoding::KIND_CDR_PLAIN_ZERO, ENDIAN_BIG);
...
Spdp::SpdpTransport::dispose_unregister()
{
  ...
  wbuff_.reset();
  DCPS::Serializer ser(&wbuff_, Encoding::KIND_CDR_PLAIN, ENDIAN_NATIVE);
...
#ifdef OPENDDS_SECURITY
    // Assume STUN
    DCPS::Serializer serializer(&buff_, Encoding::KIND_CDR_UNALIGNED, ENDIAN_BIG);
...
# endif
  ...
  DCPS::Serializer ser(&buff_, Encoding::KIND_CDR_PLAIN, ENDIAN_NATIVE);
  Header header;
  if (!(ser >> header)) {
    ACE_ERROR((LM_ERROR,
...
Spdp::SendStun::execute()
{
  ...
  tport_->wbuff_.reset();
  DCPS::Serializer serializer(&tport_->wbuff_, Encoding::KIND_CDR_UNALIGNED, ENDIAN_BIG);
