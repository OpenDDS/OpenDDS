    ...
    if (header.message_id_ == OpenDDS::DCPS::SAMPLE_DATA)
    {
          ...
          this->time_based_filter_instance(instance_ptr, filter_time_remaining))
      {
        ...
        if (this->qos_.reliability.kind == DDS::RELIABLE_RELIABILITY_QOS) {
          filterDelayedHandler_.delay_sample(handle, instance_data, header, just_registered, filter_time_remaining);
      ...
      else
      {
        ...
        filterDelayedHandler_.clear_sample(handle);
      }
...
class FilterDelayedHandler : public ACE_Event_Handler {
public:
  ...
  void delay_sample(long handle,
    ...
    ACE_DEBUG((LM_DEBUG,
      ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::delay_sample -")
      ACE_TEXT("handle=%d.\n"), handle));
    ...
    if (result.second)
    {
      ...
      sample.timer_id = dataReaderImpl_.get_reactor()->schedule_timer(this, key_ptr, filter_time_remaining, interval);
      ACE_DEBUG((LM_DEBUG,
        ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::delay_sample -")
        ACE_TEXT("timer_id=%d, filter_time_remaining=%Q, handle=%d.\n"), sample.timer_id, filter_time_remaining.sec(), handle));
    }
    else
    {
      ACE_DEBUG((LM_DEBUG,
        ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::delay_sample -")
        ACE_TEXT("replaced handle=%d.\n"), handle));
  ...
  void clear_sample(long handle) {
    ACE_DEBUG((LM_DEBUG,
      ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::clear_sample -")
      ACE_TEXT("handle=%d.\n"), handle));
    FilterDelayedSampleMap::iterator sample = map_.find(handle);
    if (sample != map_.end())
    {
  ...
  int handle_timeout(const ACE_Time_Value& current_time, const void* arg) {
    ACE_GUARD_RETURN(ACE_Recursive_Thread_Mutex, guard, dataReaderImpl_.sample_lock_,
      -1);

    const long handle = *(reinterpret_cast<const long*>(arg));
    ACE_DEBUG((LM_DEBUG,
      ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::handle_timeout -")
      ACE_TEXT("handle=%d.\n"), handle));
    FilterDelayedSampleMap::iterator data = map_.find(handle);
    if (data != map_.end())
    {
      if (data->second.message)
      {
        SubscriptionInstance* const instance_ptr = dataReaderImpl_.get_handle_instance(handle);
        ACE_DEBUG((LM_DEBUG,
          ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::handle_timeout -")
          ACE_TEXT("content handle=%d.\n"), handle));
        ...
        dataReaderImpl_.finish_store_instance_data(instance_data, *data->second.header, instance_ptr, NOT_DISPOSE_MSG, NOT_UNREGISTER_MSG);

        dataReaderImpl_.accept_sample_processing(instance_ptr, *data->second.header, data->second.new_instance);

        ACE_DEBUG((LM_DEBUG,
          ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::handle_timeout -")
          ACE_TEXT("content, done handle=%d.\n"), handle));
        return 0;
      }
      else {
        ACE_DEBUG((LM_DEBUG,
          ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::handle_timeout -")
          ACE_TEXT("no content, erase data handle = %d.\n"), handle));
        // no data to process, so remove from container
        map_.erase(data);
      }
    }

    // nothing to process, so unregister this handle for timeout
    dataReaderImpl_.get_reactor()->cancel_timer(handle);
    ACE_DEBUG((LM_DEBUG,
      ACE_TEXT("OpenDDS (%P|%t) FilterDelayedHandler::handle_timeout -")
      ACE_TEXT("unregistered handle=%d.\n"), handle));
...
FilterDelayedHandler filterDelayedHandler_;
