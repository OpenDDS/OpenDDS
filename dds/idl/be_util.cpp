...
be_util::usage()
{
  ...
  ACE_DEBUG((LM_DEBUG,
    ...
    ACE_TEXT(" --default-autoid\t\t\tset default XTypes autoid approach (sequential if not set)\n")
    ACE_TEXT(" --default-try-construct\t\tset default XTypes try-construct behavior")
    ACE_TEXT(" (discard if not set)\n")
