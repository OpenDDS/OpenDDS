...
namespace {
                            ...
                            const std::string&)
  {
    ...
    const bool must_understand = be_global->is_must_understand(branch) || be_global->is_key(branch);
    be_global->impl_ <<
      "    if (!value_writer.begin_union_member(MemberParam(" << id << ", " <<
      (must_understand ? "true" : "false") << ", \"" << field_name << "\", false, true))) {\n"
      "      return false;\n"
      "    }\n";
                                      ...
                                      const char*)
{
  ...
  const std::string type_name = scoped(name);
  {
    ...
    for (size_t i = 0; i != contents.size(); ++i) {
      ...
      const std::string idl_name = contents[i]->local_name()->get_string();
      be_global->impl_ <<
                                        ...
                                        const char*)
{
...

  {
    ...
    for (std::vector<AST_Field*>::const_iterator pos = fields.begin(), limit = fields.end();
      ...
      const bool must_understand = be_global->is_must_understand(field) || be_global->is_key(field);
      be_global->impl_ <<
                                       ...
                                       const char*)
{
...

  {
    ...
    be_global->impl_ <<
      ...
      "    return false;\n"
      "  }\n";
    const bool must_understand = be_global->is_must_understand(discriminator) || be_global->is_key(discriminator);
    be_global->impl_ <<
